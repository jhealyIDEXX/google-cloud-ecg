  ...
  def on_epoch_begin(self, epoch, logs={}):
    if epoch > 0 and epoch % self.eval_frequency == 0:
      ...
      if len(checkpoints) > 0:
        ...
        ecg_model = model.compile_model(ecg_model, self.learning_rate)
        loss, acc = ecg_model.evaluate_generator(
            model.generator_input(self.eval_files, chunk_size=CHUNK_SIZE),
            steps=self.steps)
        print('\nEvaluation epoch[{}] metrics[{:.2f}, {:.2f}] {}'.format(
            epoch, loss, acc, ecg_model.metrics_names))
...
def dispatch(train_files,
             ...
             num_epochs,
             checkpoint_epochs):
  ecg_model = model.model_fn(INPUT_SHAPE, CLASS_SIZE)
  ...
  ecg_model.fit_generator(
      model.generator_input(train_files),
