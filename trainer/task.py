	...
	def on_epoch_begin(self, epoch, logs={}):
	  ...
	  # Force snapshots and saves to local filesystem, then copy them over to GCS.
			...
			if not self.job_dir.startswith('gs://'):
				model_path_glob = os.path.join(self.job_dir,
						model_path_glob)
			checkpoints = glob.glob(model_path_glob)
			if len(checkpoints) > 0:
				checkpoints.sort()
				ecg_model = load_model(checkpoints[-1])
				ecg_model = model.compile_model(ecg_model,
						self.learning_rate)
				(loss, acc) = \
					ecg_model.evaluate_generator(model.generator_input(self.eval_files,
						chunk_size=CHUNK_SIZE), steps=self.steps)
				print('\nEvaluation epoch[{}] metrics[{:.2f}, {:.2f}] {}'.format(epoch, loss, acc, ecg_model.metrics_names))
...
def dispatch(
  ...
  # Tensorboard logs callback
	...
	if train_files.endswith('txt'):
		if train_files.startswith('gs://'):
			with tf.gfile.Open(train_files, 'rb') as f:
				X = np.load(f)
			with tf.gfile.Open(label_files, 'rb') as f:
				Y = np.load(f)
		else:
...
if __name__ == '__main__':
	...
	parser.add_argument('--train-files', required=True, type=str,
						help='Training files local or GCS', nargs='+')
	parser.add_argument('--eval-files', required=True, type=str,
	...
	parser.add_argument('--eval-batch-size', type=int, default=40,
						help='Batch size for evaluation steps')
	parser.add_argument('--learning-rate', type=float, default=0.003,
						help='Learning rate for SGD')
	parser.add_argument('--eval-frequency', default=10,
						help='Perform one evaluation per n epochs')
	parser.add_argument('--first-layer-size', type=int, default=256,
	...
	parser.add_argument('--eval-num-epochs', type=int, default=1,
